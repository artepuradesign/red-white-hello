...
const SidebarMenu: React.FC<SidebarMenuProps> = ({
  ...
  React.useEffect(() => {
    ...
    // Expande o submenu de 2º nível correspondente (ex.: "Histórico")
    const subParentsToExpand: Array<{ parentLabel: string; childLabel: string }> = [];
    filteredItems.forEach((it) => {
      it.subItems?.forEach((sub) => {
        const hasNestedActive = sub.subItems?.some((nested) => nested.path === location.pathname);
        if (hasNestedActive) {
          subParentsToExpand.push({ parentLabel: it.label, childLabel: sub.label });
        }
      });
    });

    if (subParentsToExpand.length > 0) {
      setExpandedSubItems((prev) => {
        const next = new Set(prev);
        subParentsToExpand.forEach(({ parentLabel, childLabel }) => next.add(makeSubKey(parentLabel, childLabel)));
        return next;
      });
    }
